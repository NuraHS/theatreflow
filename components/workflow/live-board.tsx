    ...
    <div className="space-y-4">
      ...
      <Card className="border-cyan-300 dark:border-cyan-900">
        ...
        <CardContent className="space-y-4">
          <div className="flex gap-2 overflow-x-auto pb-1" aria-label="Theatre selector">
            <Button type="button" variant={selectedTheatreId === "all" ? "default" : "outline"} className="min-h-11 shrink-0" onClick={() => setSelectedTheatreId("all")}>
              All theatres
              <Badge tone={selectedTheatreId === "all" ? "blue" : "neutral"}>{selectedPatients.length}</Badge>
            </Button>
            {suiteTheatres.map((theatre) => {
              const count = livePatients.filter((patient) => patient.theatre_id === theatre.id).length;
              return (
                <Button key={theatre.id} type="button" variant={selectedTheatreId === theatre.id ? "default" : "outline"} className="min-h-11 shrink-0" onClick={() => setSelectedTheatreId(theatre.id)}>
                  {theatre.name}
            ...
            {unassignedCount ? (
              <Button type="button" variant={selectedTheatreId === "unassigned" ? "default" : "outline"} className="min-h-11 shrink-0" onClick={() => { setSelectedSuiteId("all"); setSelectedTheatreId("unassigned"); }}>
                Unassigned
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
            {suiteTheatres.map((theatre) => {
              ...
              const suite = locations.suites.find((item) => item.id === theatre.suite_id);
              ...
              return (
                <button key={theatre.id} type="button" onClick={() => setSelectedTheatreId(theatre.id)} className={cn("cursor-pointer rounded-lg border p-4 text-left transition-colors duration-200 hover:border-primary hover:bg-muted focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-ring/30", selectedTheatreId === theatre.id && "border-primary bg-cyan-50 dark:bg-cyan-950/25")}>
      ...
      <Card>
        ...
        <CardContent className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {selectedRecoveryAreas.map((area) => {
            ...
            return (
              <section key={area.id} className="rounded-lg border p-4">
